...
def save(obj: dict, checkpoint_file: str) -> None:
    ...
    if backend is None:
        ...
        _saver = PyTorchH5Saver()
        _saver.save(checkpoint_file, obj)
    else:
...
def _cstorch_load(
    ...
    **kwargs,
    ...
    _saver = PyTorchH5Saver()
    ckpt_version = _saver.extract_version(checkpoint_file)

    with cstorch.saver.storage.cache_deferred_tensors(cache_tensors):
        values = []
        vals = _saver.load(checkpoint_file, present)
